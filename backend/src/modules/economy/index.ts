...
import { refundAbandonedBlackjackStakes } from "./funds.js";
...
export const economyModule: AdobosModule = {
  ...
  register(ctx) {
    ...
    ctx.button(CF_BUTTON_PREFIX, (interaction) => handleCoinflipButton(interaction));
    ctx.button(RL_BUTTON_PREFIX, (interaction) => handleRouletteButton(interaction));
    ctx.button(SL_BUTTON_PREFIX, (interaction) => handleSlotsButton(interaction));
    ctx.select(RL_SELECT_PREFIX, (interaction) => handleRouletteSelect(interaction));
    ctx.select(WK_SELECT_PREFIX, (interaction) => handleWorkSelect(interaction));
    ctx.select(CR_SELECT_PREFIX, (interaction) => handleCrimeSelect(interaction));
